...
class NetworkConstants:
    @classmethod
    def set_mainnet(cls, coinType):
        cls.COIN_TYPE = coinType
        cls.TESTNET = False
        if coinType == 'BTC':
            cls.WIF_PREFIX = 0x80
            ...
            cls.ADDRTYPE_P2SH = 5
            cls.SEGWIT_HRP = "bc"
            cls.SCRIPT_TYPES = SCRIPT_TYPES_BTC
            cls.MSG_MAGIC = b"\x18Bitcoin Signed Message:\n"
        elif coinType == 'LTC':
            cls.WIF_PREFIX = 0x80
            cls.ADDRTYPE_P2PKH = 48
            cls.ADDRTYPE_P2SH = 50
            cls.SEGWIT_HRP = "ltc"
            cls.SCRIPT_TYPES = SCRIPT_TYPES_LTC
            cls.MSG_MAGIC = b"\x19Litecoin Signed Message:\n"

    @classmethod
    def set_testnet(cls,coinType):
        cls.COIN_TYPE = coinType
        cls.TESTNET = True
        if coinType == 'BTC':
            cls.WIF_PREFIX = 0xef
...
#NetworkConstants.set_mainnet('BTC')
...
def serialize_privkey(secret, compressed, txin_type):
    prefix = bytes([(NetworkConstants.SCRIPT_TYPES[txin_type]+NetworkConstants.WIF_PREFIX)&255])
...
def deserialize_privkey(key):
    ...
    if is_minikey(key):
        ...
        txin_type = inv_dict(NetworkConstants.SCRIPT_TYPES)[vch[0] - NetworkConstants.WIF_PREFIX]
...
def msg_magic(message):
    ...
    return NetworkConstants.MSG_MAGIC + length + message
